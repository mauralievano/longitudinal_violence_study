...
for(t in c(16:18, 20:22, 24)){
  ...
  F_index <- which(is.na(data[, t]) & data[, t-9] == F & data$C2 == 0)
...
for(t in c(34:36, 38:40, 42)){
  T_index <- which(is.na(data[, t]) & (data[, t-9] == T | data[, t-18] == T | data[, t-27] == T) & data$C4 == 0)
  T_prob <- sum(data[, t] == T & (data[, t-9] == T | data[, t-18] == T | data[, t-27] == T), 
                na.rm = T) / sum(data[, t-9] == T | data[, t-18] == T | data[, t-27] == T, na.rm = T)
  data[T_index, t] <- rbinom(length(T_index), 1, T_prob)
  
  F_index <- which(is.na(data[, t]) & data[, t-9] == F & data[, t-18] == F & data[, t-27] == F  & data$C4 == 0)
...
# # restore censoring
# data[which(data$C2 == 1), c(16:22, 24:31, 33:40, 42:43)] <- NA
# data[which(data$C3 == 1), c(25:31, 33:40, 42:43)] <- NA
# data[which(data$C4 == 1), c(34:40, 42:43)] <- NA
@
